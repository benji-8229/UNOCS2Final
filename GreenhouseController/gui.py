...
class GreenhouseGUI(object):
    """
    GUI controller for greenhouse control unit. Controls placement of labels, text fields, buttons, and dynamically placing them.
    """

    small_font = QtGui.QFont("Times", 9)
    big_font = QtGui.QFont("Times", 14)
    default_height = 24
    ...
    def setupUI(self, main_window: QtWidgets.QMainWindow, dec_width: int, dec_height: int) -> None:
        ...
        __, self.humidity_field = self.create_label_field("% Relative Humidity", side_offset, top_offset, 100, GreenhouseGUI.default_height)
        ...
        self.temp_label, self.temp_field = self.create_label_field("Temperature", int(dec_width / 2) - 50, top_offset, 100, GreenhouseGUI.default_height)
        ...
        __, self.co2_field = self.create_label_field("Co2 PPM", dec_width - 100 - side_offset, top_offset, 100, GreenhouseGUI.default_height)
        ...
        self.light_off_field = self.create_label_field_side("Light Off Time", side_offset, top_offset + int(GreenhouseGUI.default_height * 1.1))
        # Photo gui
        # Start position will be 1.2 * the end position of our light settings
        top_offset_photos = int((top_offset + GreenhouseGUI.default_height * 1.1 + 33) * 1.2)
    ...
    def create_label_field(self, label_text: str, x: int, y: int, width: int, height: int) -> tuple:
        """
        Generates a line edit field with label centered above
        :param label_text: Text to be above label
        ...
        label.setFont(GreenhouseGUI.small_font)
        ...
        field.setFont(GreenhouseGUI.small_font)
    ...
    def create_label_field_side(self, label_text: str, x: int, y: int) -> QtWidgets.QLineEdit:
        ...
        return_field.setFixedSize(75, GreenhouseGUI.default_height)
        ...
        tmp_label.setFixedHeight(GreenhouseGUI.default_height)
